...
class Map
{
    private:
        ...
        static const int max_npcs_ = 10;  // max non-player characters
        static const int max_rooms_ = 10; // max number of rooms
    ...
    public:
        ...
        bool addExit(int row, int col);
        bool removeNPC(int row, int col);
        bool removeRoom(int row, int col);
        void exploreSpace(int row, int col);
        void moveActionMenu();
        void movementFunction();
        void normalActionMenu();
        // void normalActionFunction(int, int, int, int, int);
        // int fightFunction(int, int, int, int, int); // return val == if someone dies or if they killed the monster
        // void investigateFunction();
